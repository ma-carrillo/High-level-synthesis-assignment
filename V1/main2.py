...
# =========================
# VHDL Generator: fill FSM case statements (control)
# =========================

class VHDLGeneratorWithControl(VHDLGenerator):
    """
    Extends the previous VHDLGenerator to *fill* the FSM control based on control_by_t.
    """

    def generate_with_control(self, dp, schedule, binding, edge_regs, dp_info, top_name="hls_top"):
        # Generate base VHDL (skeleton) then patch the TODOs by regenerating FSM cleanly.
        # To keep things simple and deterministic, we generate VHDL from scratch here.

        # Build control map
        ctrl_gen = ControlGenerator()
        control_by_t = ctrl_gen.build_controls(
            dfg=self._infer_dfg_from_binding(binding),  # not used; we pass actual dfg below in real integration
            schedule=schedule,
            binding=binding,
            edge_regs=edge_regs,
            dp_info=dp_info
        )
        # NOTE: Above line won't work as-is because binding alone isn't enough to reconstruct DFG.
        # In your real call, pass the DFG explicitly to build_controls, then pass control_by_t here.
        #
        # So: use the next helper method below instead of this wrapper, or integrate directly.

        raise NotImplementedError("Use generate_full(...) below which takes DFG explicitly.")

    def generate_full(self, dfg, dp, schedule, binding, edge_regs, dp_info, top_name="hls_top"):
        """
        Full VHDL generation including control case statements.
        Assumes:
          - reg enables are reg_name_en
          - mux selects are sel_<res.instance>_<portlabel>
          - memory ctrl signals: mem_X_en, mem_X_we, mem_X_addr, mem_X_din
        """
        w = VHDLWriter()
        ...
        w.writeln("use ieee.numeric_std.all;")
        w.writeln("")
        w.writeln(f"entity {top_name} is")
        w.indent()
        w.writeln("port(")
        w.indent()
        w.writeln("clk   : in  std_logic;")
        ...
        w.writeln(f"architecture rtl of {top_name} is")
        w.indent()

        # --- declare reg signals ---
        reg_names = sorted({reg.name for reg in dp_info["reg_node"].keys()})  # dp_info maps Register->id
        # dp_info["reg_node"] keys are Register objects; easier:
        reg_names = sorted({reg.name for reg in dp_info["reg_node"]})
        for rn in reg_names:
            w.writeln(f"signal {rn}_q  : signed(31 downto 0);")
            w.writeln(f"signal {rn}_en : std_logic;")
        w.writeln("")

        # --- declare mux sel signals ---
        mux_inputs = dp_info["mux_inputs"]
        # mux_inputs keys: (Resource, port_label)
        for (res, portlab), regs in mux_inputs.items():
            w.writeln(f"signal sel_{res.instance}_{portlab} : integer := 0;")
        w.writeln("")

        # --- memory signals (one per mem instance) ---
        mem_instances = sorted({binding[nid].instance for nid in binding if binding[nid].kind == "mem"})
        ...
        # --- component decls (assumed) ---
        w.writeln("-- Components assumed to exist")
        w.writeln("component Reg32 is")
        w.indent()
        w.writeln("port(clk: in std_logic; en: in std_logic; d: in signed(31 downto 0); q: out signed(31 downto 0));")
        w.dedent()
        ...
        w.writeln("-- TODO: instantiate datapath components and mux combinational logic")
        w.writeln("-- (you can keep your structural generator and plug it here)")
        w.writeln("")
        ...
        # --- FSM process with explicit control ---
        w.writeln("process(clk)")
        w.writeln("begin")
        w.indent()
        w.writeln("if rising_edge(clk) then")
        ...
        # defaults (set everything to 0)
        w.writeln("-- defaults each cycle")
        for rn in reg_names:
            w.writeln(f"{rn}_en <= '0';")
        for inst in mem_instances:
            w.writeln(f"{inst}_en <= '0';")
            w.writeln(f"{inst}_we <= '0';")
        w.writeln("done <= '0';")
        w.writeln("")
        ...
        for t in range(0, max_t + 1):
            ...
            if ctrl:
                ...
                for inst in sorted(ctrl["mem_en"]):
                    ...
                    # addr reg drives mem addr
                    addr_r = ctrl["mem_addr_reg"].get(inst, None)
                    if addr_r is not None:
                        w.writeln(f"{inst}_addr <= {addr_r}_q;")

                    # din for stores
                    din_r = ctrl["mem_din_reg"].get(inst, None)
                    if din_r is not None:
                        w.writeln(f"{inst}_din <= {din_r}_q;")
...
# =========================
# Demo: FULL unified VHDL + print_datapath(dp)
# =========================
...
if __name__ == "__main__":
    ...
    prog = Store(
        ...
        Cst(3),
        Add(
            Mul(Load(ram, Cst(0)), Load(ram, Cst(1))),
            Load(ram, Cst(2)),
        ),
    )
    ...
    # 5) Datapath build (MUST be the UPDATED builder that records mux_inputs)
    dp_builder = DatapathBuilder()
    dp, dp_info = dp_builder.build(dfg, schedule, binding, edge_regs)

    # 6) Verify datapath graph
    print("\n=== DATAPATH GRAPH (VERIFY) ===")
    print_datapath(dp)

    # Quick sanity check: ensure mux_inputs exists and has entries
    ...
    # dp_info["mux_inputs"] keys: (Resource, port_label) -> [reg_name0, reg_name1, ...]
    for (res, portlab), reg_list in dp_info["mux_inputs"].items():
        print(f"  mux for {res.instance}.{portlab}: {reg_list}")

    # 7) Generate FULL unified VHDL (datapath + muxes + FSM control)
