...
if __name__ == "__main__":
    ...
    prog = Block([Store(
        ...
        Add(
            Add(
                Add(
                    Mul(Load(A, Cst(0)), Load(B, Cst(0))),
                    Mul(Load(A, Cst(1)), Load(B, Cst(1))),
                ),
                Mul(Load(A, Cst(2)), Load(B, Cst(2))),
            ),
            Mul(Load(A, Cst(3)), Load(B, Cst(3))),
        ),
    )])
    ...
    gen = UnifiedVHDLGenerator(top_name="hls_top_unified_02")
    vhdl = gen.generate_full(dfg, dp, schedule, binding, edge_regs, dp_info)

    write_text("hls_top_unified_02.vhd", vhdl)

    print(f"\nWrote: {out_path('hls_top_unified_02.vhd')}")
