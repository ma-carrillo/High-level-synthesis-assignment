...
from hls_core_part4_2 import (
    ...
    print_datapath, DatapathDotPrinter, Assign, Var, DFGOptimizer
)
...
if __name__ == "__main__":
    ...
    #  OPTIMIZATIONS
    dfg = DFGOptimizer(enable_cse=True, enable_chaining=True).run(dfg)
    ...
    gen = UnifiedVHDLGenerator(top_name="hls_top_unified_20")
    vhdl = gen.generate_full(dfg, dp, schedule, binding, edge_regs, dp_info)

    write_text("hls_top_unified_20.vhd", vhdl)

    print(f"\nWrote: {out_path('hls_top_unified_20.vhd')}")
