...
# =========================
# Demo: FULL unified VHDL + print_datapath(dp)  (Block support)
# =========================

if __name__ == "__main__":
    # One RAM: keep it simple
    ram = Mem(1024, init=[7, 5, 11, 0, 0])

    # Two blocks total:
    #   - Outer Block has:
    #       1) Store #1
    #       2) Nested Block with Store #2
    #
    # Assumption per assignment: statements are "independent" (no explicit deps between stores).
    prog = Block([
        # Store #1: ram[3] = ram[0] + 1
        Store(
            ram,
            Cst(3),
            Add(Load(ram, Cst(0)), Cst(1)),
        ),

        # Block #2 (nested): ram[4] = ram[1] * 2
        Block([
            Store(
                ram,
                Cst(4),
                Mul(Load(ram, Cst(1)), Cst(2)),
            )
        ]),
    ])
    ...
    # (Optional) show schedule + binding + reg allocation to confirm both stores exist in the DFG
    ...
    # 5) Datapath build (MUST be UPDATED builder that records mux_inputs)
